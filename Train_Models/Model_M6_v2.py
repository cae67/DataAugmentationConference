...
# Fourier Transform Shift Augmentation
from scipy.signal import hilbert

def ft_shift(X_train,fs,delta_f_max,seed):
    # X_train - data in shape n_samples x n_timepoints x n_channels
    # delta_f_max - max frequency shift in Hertz
    # fs - sampling rate in Hertz
    
    # Get Data Dimensions
    n_samples, n_timepoints, n_channels = np.shape(X_train)
    
    # Get Copy of Data
    X_train = X_train.copy()
    
    # Get Analytical Signal
    X_train = hilbert(X_train,axis=1)
        
    # Get Timepoints Used in Shift
    t = np.expand_dims(np.arange(n_timepoints)/fs,axis=(0,2))
    t = np.repeat(np.repeat(t,n_samples,axis=0),n_channels,axis=2)
    
    # Create Shift
    np.random.seed(seed)
    shift = np.random.uniform(-1,1,size=(n_samples, 1))
    shift = np.repeat(shift*2*np.pi*delta_f_max,n_timepoints,axis=1) # duplicate for each frequency
    shift = np.repeat(np.expand_dims(shift,axis=2),19,axis=2)
    
    # Get Shifted Signal
    X_train = np.real(X_train * np.exp(shift*1j*t))
...
## Model 6.2: Model Trained with 2 Copies of FT Shift Augmented Data
...
class CVTuner(kt.Hyperband):
    def run_trial(self, trial, x, y, groups, save_path, count, batch_size=128, epochs=10,augmentation_param=0.3):
        ...
        for tv_idx, test_idx in gss.split(x, y, groups):
            ...
            X_train = np.vstack((X_train, ft_shift(X_train,200,augmentation_param,3), ft_shift(X_train,200,augmentation_param,4)))
...
# Fourier Transform Shift Parameters Values
param_vals = np.arange(1,11)*3/10 # max frequency of shift, between 0 and 3 Hz

# Initialize File for Saving Scores
save_path_val_logs = save_path +'val_scores_m6_v2.mat'
savemat(save_path_val_logs, {'val_accuracy':np.zeros_like(param_vals)})

# Iteratively Train on Different Augmentation Parameters
count = 0
...
for tv_idx, test_idx in gss.split(data, labels, groups):
    ...
    # Perform Data Augmentation
    X_train = np.vstack((X_train, ft_shift(X_train,200,best_param,3), ft_shift(X_train,200,best_param,4)))
    y_train = np.hstack((y_train, y_train, y_train))
    ...
    save_model_path = save_path + "Models/model_m6_v2_fold"+str(i)+".hdf5"
...
results_filename = save_path + "Performance/model_m6_v2.mat"
