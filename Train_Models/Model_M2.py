...
## Define Base Model with Optimal Hyperparameters from M1.1 and Variable Learning Rate
...
## Model 2.1 - Model with 1 Copy of Gaussian Noise Augmented Data
...
class CVTuner(kt.Hyperband):
    def run_trial(self, trial, x, y, groups, save_path, count, batch_size=128, epochs=10,augmentation_param=0.3):
        ...
        for tv_idx, test_idx in gss.split(x, y, groups):
            ...
            X_train = np.vstack((X_train, gaussian_noise_augmentation(X_train,augmentation_param,3)))
...
# Initialize List for Storing Optimal Hyperparameter Values
hps = []
...
# Iterate over Tuner for Each Augmentation Parameter Value
count = 0
...
for tv_idx, test_idx in gss.split(data, labels, groups):
    ...
    # Perform Data Augmentation
    X_train = np.vstack((X_train, gaussian_noise_augmentation(X_train,best_param,3)))
    y_train = np.hstack((y_train, y_train))
    ...
    save_model_path = save_path + "Models/model_m2_fold"+str(i)+".hdf5"
...
results_filename = save_path + "Performance/model_m2.mat"
