...
## Model 6.1: Model Trained with 1 Copy of FT Shift Augmented Data
...
class CVTuner(kt.Hyperband):
    def run_trial(self, trial, x, y, groups, save_path, count, batch_size=128, epochs=10,augmentation_param=0.3):
        ...
        for tv_idx, test_idx in gss.split(x, y, groups):
            ...
            X_train = np.vstack((X_train, ft_shift(X_train,200,augmentation_param,3)))
...
save_path_val_logs = save_path +'val_scores_m6.mat'
savemat(save_path_val_logs, {'val_accuracy':np.zeros_like(param_vals)})

# Iteratively Train on Different Optimal Hyperparameters
count = 0
...
for tv_idx, test_idx in gss.split(data, labels, groups):
    ...
    # Perform Data Augmentation
    X_train = np.vstack((X_train, ft_shift(X_train,200,best_param,3)))
    y_train = np.hstack((y_train, y_train))
    ...
    save_model_path = save_path + "Models/model_m6_fold"+str(i)+".hdf5"
...
results_filename = save_path + "Performance/model_m6.mat"
