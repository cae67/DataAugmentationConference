...
# Channel Dropout Data Augmentation
from scipy.stats import bernoulli

def channel_dropout_augmentation(X_train,dropout_rate,seed):
    # X_train - data in shape n_samples x n_timepoints x n_channels
    # dropout_rate - probability that a given channel will be dropped out
    ...
    # Make Mask of Channels to Dropout
    mask = bernoulli.rvs(dropout_rate,size=(n_samples,n_channels))
    mask = np.expand_dims(mask,axis=1)
    mask = np.repeat(mask,n_timepoints,axis=1)
...
## Model 7.2: Model Trained on 2 Copies of Channel Dropout Augmented Data
...
class CVTuner(kt.Hyperband):
    def run_trial(self, trial, x, y, groups, save_path, count, batch_size=128, epochs=10,augmentation_param=0.3):
        ...
        for tv_idx, test_idx in gss.split(x, y, groups):
            ...
            X_train = np.vstack((X_train, channel_dropout_augmentation(X_train,augmentation_param,3), channel_dropout_augmentation(X_train,augmentation_param,4)))
...
# Channel Dropout Parameter Values
param_vals = np.arange(1,10)/10

# Initialize File for Saving Scores
save_path_val_logs = save_path +'val_scores_m7_v2.mat'
savemat(save_path_val_logs, {'val_accuracy':np.zeros_like(param_vals)})

# Iteratively Train on Augmentation Paramters
count = 0
...
for tv_idx, test_idx in gss.split(data, labels, groups):
    ...
    # Perform Data Augmentation
    X_train = np.vstack((X_train, channel_dropout_augmentation(X_train,best_param,3), channel_dropout_augmentation(X_train,best_param,4)))
    y_train = np.hstack((y_train, y_train, y_train))
    ...
    save_model_path = save_path + "Models/model_m7_v2_fold"+str(i)+".hdf5"
    ...
    # Train Model
    history = model.fit(X_train, to_categorical(y_train), epochs= 10, batch_size = 128, validation_data=(X_val, to_categorical(y_val)), 
...
results_filename = save_path + "Performance/model_m7_v2.mat"
