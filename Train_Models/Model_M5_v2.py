...
def ft_surrogate(X_train,fs,phase_shift_max,seed):
    # X_train - data in shape n_samples x n_timepoints x n_channels
    # fs - sampling rate of data
    # phase_shift_max - random phase perturbation between 0 and 2*pi
    ...
    # Perform FFT
    fft_vals = np.fft.fft(X_train,axis=1)
    
    # Get Random Phase
    np.random.seed(seed)
    rand_phase = np.random.uniform(0,1,size=(n_samples, n_timepoints))*phase_shift_max
    rand_phase = np.repeat(np.expand_dims(rand_phase,axis=2),19,axis=2)
    
    # Perturb with Random Phase
    fft_vals = fft_vals * np.exp(rand_phase*-1j)
    
    # Perform iFFT
    X_train = np.fft.ifft(fft_vals,axis=1)
...
## Model 5.2: Model Trained with 2 FT Surrogate Augmented Copies of Training Data
...
class CVTuner(kt.Hyperband):
    def run_trial(self, trial, x, y, groups, save_path, count, batch_size=128, epochs=10,augmentation_param=0.3):
        ...
        for tv_idx, test_idx in gss.split(x, y, groups):
            ...
            X_train = np.vstack((X_train, ft_surrogate(X_train,200,augmentation_param,3), ft_surrogate(X_train,200,augmentation_param,4)))
...
# Fourier Transform Surrogate Parameters Values
param_vals = np.arange(1,11)*4*np.pi/20 # between 0 and 2pi

# Initialize File for Saving Scores
save_path_val_logs = save_path +'val_scores_m5_v2.mat'
savemat(save_path_val_logs, {'val_accuracy':np.zeros_like(param_vals)})

# Iteratively Train with Augmentation Parameters
count = 0
...
for tv_idx, test_idx in gss.split(data, labels, groups):
    ...
    # Perform Data Augmentation
    X_train = np.vstack((X_train, ft_surrogate(X_train,200,best_param,3), ft_surrogate(X_train,200,best_param,4)))
    y_train = np.hstack((y_train, y_train, y_train))
    ...
    save_model_path = save_path + "Models/model_m5_v2_fold"+str(i)+".hdf5"
...
results_filename = save_path + "Performance/model_m5_v2.mat"
