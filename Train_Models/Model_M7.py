...
## Model 7.1: Model Trained on 1 Copy of Channel Dropout Augmented Data
...
class CVTuner(kt.Hyperband):
    def run_trial(self, trial, x, y, groups, save_path, count, batch_size=128, epochs=10,augmentation_param=0.3):
        ...
        for tv_idx, test_idx in gss.split(x, y, groups):
            ...
            X_train = np.vstack((X_train, channel_dropout_augmentation(X_train,augmentation_param,3)))
...
# Initialize List for Storing Best Hyperparameters
hps = []
...
save_path_val_logs = save_path +'val_scores_m7.mat'
savemat(save_path_val_logs, {'val_accuracy':np.zeros_like(param_vals)})

# Iteratively Train on Different Augmentation Parameter Values
count = 0
...
for tv_idx, test_idx in gss.split(data, labels, groups):
    ...
    # Perform Data Augmentation
    X_train = np.vstack((X_train, channel_dropout_augmentation(X_train,best_param,3)))
    y_train = np.hstack((y_train, y_train))
    ...
    save_model_path = save_path + "Models/model_m7_fold"+str(i)+".hdf5"
...
results_filename = save_path + "Performance/model_m7.mat"
