...
## Define Base Model with M1.1 Hyperparameters and Variable Learning Rate
...
def smooth_time_augmentation(X_train,delta_t,lambda_val,seed):
...
## Model 4.2: Model Trained with 2 Copies of Smooth Time Masking Augmented Data
...
class CVTuner(kt.Hyperband):
    def run_trial(self, trial, x, y, groups, save_path, count, batch_size=128, epochs=10,augmentation_param=0.3):
        ...
        for tv_idx, test_idx in gss.split(x, y, groups):
            ...
            X_train = np.vstack((X_train, smooth_time_augmentation(X_train,augmentation_param,0.5,seed=1), smooth_time_augmentation(X_train,augmentation_param,0.5,seed=2)))
...
# Initialize List to Store Optimal Hyperparameters
hps = []
...
for tv_idx, test_idx in gss.split(data, labels, groups):
    ...
    # Perform Data Augmentation
    X_train = np.vstack((X_train, smooth_time_augmentation(X_train,best_param,0.5,seed=1), smooth_time_augmentation(X_train,best_param,0.5,seed=2)))
    y_train = np.hstack((y_train, y_train, y_train))
    ...
    save_model_path = save_path + "Models/model_m4_v2_fold"+str(i)+".hdf5"
...
results_filename = save_path + "Performance/model_m4_v2.mat"
