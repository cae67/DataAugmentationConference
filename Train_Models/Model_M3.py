...
def time_reverse_augmentation(X_train):
    
    # Flip Along Time Axis.
    # Assumes Dimensions are (N_Samples x N_TimePoints x N_Channels).
    X_train = np.flip(X_train.copy(),axis=1)
...
## Model 3.1: Model with Time Reverse Augmentation
...
class CVTuner(kt.Hyperband):
    def run_trial(self, trial, x, y, groups, batch_size=128, epochs=10):
        ...
        for tv_idx, test_idx in gss.split(x, y, groups):
            ...
            # Duplicate Data
            X_train = np.vstack((X_train, time_reverse_augmentation(X_train)))
...
# Define List for Storing Optimal Hyperparameters
hps = []
...
for tv_idx, test_idx in gss.split(data, labels, groups):
    ...
    save_model_path = save_path + "Models/model_m3_fold"+str(i)+".hdf5"
...
results_filename = save_path + "Performance/model_m3.mat"
