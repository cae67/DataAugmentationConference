...
def gaussian_noise_augmentation(X_train,noise,seed):
...
## Model 2.2: Model with 2 Copies of Gaussian Noise Augmented Data
...
class CVTuner(kt.Hyperband):
    def run_trial(self, trial, x, y, groups, save_path, count, batch_size=128, epochs=10,augmentation_param=0.3):
        ...
        for tv_idx, test_idx in gss.split(x, y, groups):
            ...
            X_train = np.vstack((X_train, gaussian_noise_augmentation(X_train,augmentation_param,3), gaussian_noise_augmentation(X_train,augmentation_param,4)))
...
# Initialize List for Storing Hyperparameter Values
hps = []

# Gaussian Noise Augmentation Parameter Values
param_vals = np.arange(1,11)/10

# Initialize File for Saving Scores
save_path_val_logs = save_path +'val_scores_m2.mat'
...
# Get Best Hyperparameters Across Augmentation Parameter Values
...
for tv_idx, test_idx in gss.split(data, labels, groups):
    ...
    # Perform Data Augmentation
    X_train = np.vstack((X_train, gaussian_noise_augmentation(X_train,best_param,3), gaussian_noise_augmentation(X_train,best_param,4)))
    y_train = np.hstack((y_train, y_train, y_train))
    ...
    save_model_path = save_path + "Models/model_m2_v2_fold"+str(i)+".hdf5"
...
results_filename = save_path + "Performance/model_m2_v2.mat"
